...
assert(app.includes('data-settings-pane="connection"><span>${tabConnectionSvg}</span>') && app.includes('data-settings-pane="backend"><span>${tabBackendSvg}</span>') && app.includes('data-settings-pane="appearance"><span>${tabAppearanceSvg}</span>') && app.includes('data-settings-pane="keybinds"><span>${tabKeybindsSvg}</span>'), 'Desktop Settings tabs must use the same SVG icon system, including Keybinds.');
